 ...
 * on disk (written by the Web UI), reloads from disk automatically.
 *
...
// ── Types (same as lib/jsonl-store.ts) ──
...
export class JsonlStore {
  ...
  getAllNodes(limit = 50): StoredNode[] { return this.nodes.getAll().slice(0, limit); }
  ...
  updateEdgeCeremony(fromId: string, toId: string, ceremonyId: string): void { this.edges.updateCeremony(fromId, toId, ceremonyId); }
  ...
  getAllBeats(limit = 50): StoredBeat[] { return this.beats.getAll().slice(0, limit); }
  ...
  archiveCycle(id: string): void {
    ...
    if (cycle) { cycle.archived = true; this.cycles.set(id, cycle); }
  }
...
/**
 * Resolve the project root .mw/store/ directory.
 * The MCP server runs from the mcp/ subdirectory, so we go up one level.
 */
function resolveProjectDataDir(): string {
  ...
  // If running from mcp/ directory, resolve to parent's .mw/store/
  if (cwd.endsWith('/mcp') || cwd.endsWith('\\mcp')) {
    return path.join(path.dirname(cwd), '.mw', 'store');
  }

  // Try to find project root by looking for package.json with "medicine-wheel" name
  let dir = cwd;
  for (let i = 0; i < 5; i++) {
