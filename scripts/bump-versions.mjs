...
// `--no-workspaces-update` because npm's post-version lockfile update resolves every
// workspace dependency range AGAINST THE REGISTRY, before sync-versions.mjs has realigned
// them. Mid-bump the ranges still name the old version while the workspaces carry the new
// one, so npm leaves the tree and asks npmjs — and for a package that has never been
// published that is a hard E404.
//
// Measured 2026-09-20, the first release carrying @medicine-wheel/honcho: all 29 lockstep
// workspaces bumped to 0.15.0, then `npm version` exited 1 on
// `@medicine-wheel/honcho@^0.14.4 — 404 Not Found`, so this script threw BEFORE the
// TRACKED_PACKAGES loop below and mcp stayed at 4.14.4 while the suite moved. That is
// exactly the "a different version line is not a reason to skip a package" failure
// RELEASING.md exists to prevent, arriving by a new route: not a skip-list, an exception.
// Every first release of a new package would hit it.
//
// The lockfile is regenerated by the `npm install` that version:* runs after
// sync-versions.mjs, which is where it belongs — after the ranges are correct.
const cmd = `npm version ${bump} ${wsFlags} --no-git-tag-version --no-workspaces-update`;
...
for (const [packageName, workspacePath] of TRACKED_PACKAGES) {
	...
	// Same reason as the lockstep call above: no registry resolution mid-bump.
	execSync(`npm version ${target} --workspace ${workspacePath} --no-git-tag-version --no-workspaces-update`, {
		stdio: 'inherit',
	});
}
